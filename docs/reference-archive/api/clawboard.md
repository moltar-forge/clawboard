...
Clawboard is a **self-hosted operating system for agent work**.

This repo (`clawboard-api`) is the **API / backend layer** within that larger system.

## The layered model

```text
Clawboard Dashboard (UI layer)  ->  Clawboard API (backend)  ->  OpenClaw (runtime + source of truth)
```

- **Clawboard Dashboard**: the UI for humans to manage tasks, see activity, browse workspaces, and operate the system.
- **Clawboard API**: the task backend + integration layer (REST endpoints for tasks/users/activity + OpenClaw workspace integration).
...
Note: the dashboard repo also contains Clawboard framing and copy; keep cross-repo wording consistent.
