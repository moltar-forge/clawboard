...
This starts Postgres, the API, and the dashboard. The dashboard runs as a **Vite dev server with hot-reload** — source changes in `web/` reflect instantly in the browser without a rebuild.
...
→ Verify `VITE_API_URL` in `web/.env` matches the running API URL.
