# Clawboard API integration

The dashboard is a UI layer. It talks to **Clawboard API**, which in turn integrates with OpenClaw services/workspaces.

## Responsibilities split

- **Dashboard**: UX, visualization, optimistic interaction where safe
- **Clawboard API**: authentication + authorization, normalization, proxying OpenClaw, enforcing invariants
...
- Clawboard API performs system-managed docs-link reconciliation through workspace-service link resources
...
- Clawboard API must allow credentials and set an explicit allowed origin (not `*`)
