...
- **Clawboard DB metadata** for display names, hierarchy (`reportsTo`), and status metadata

Clawboard also synthesizes an implicit `main` agent when it is not explicitly present in
...
Clawboard provisions:

- `<workspace>/tools/clawboard-auth`
- `<workspace>/tools/clawboard-task`
- `<workspace>/tools/INTEGRATION.md`
- `<workspace>/TOOLS.md`
- `<workspace>/BOOTSTRAP.md`
- `<workspace>/clawboard.env` (only when a new API key is created)

## Re-bootstrap action

Each editable agent card includes a **Re-bootstrap** action (circular arrows icon).

Use this when onboarding drifted or when an agent was created outside Clawboard. Re-bootstrap re-seeds
...
4. Enforces single-active-key policy:
   ...
   - writes `clawboard.env` only when a new key is created
...
- If provisioning fails before runtime config apply, Clawboard cleans up newly created key/env
  artifacts.
- If the DB row was newly created in this request and bootstrap fails before apply, Clawboard removes
...
- Re-bootstrap validates workspace roots to agent-safe paths under `/workspace` or
  `/workspace-<agent>`.
- Clawboard keeps **at most one active API key per agent**.
- Existing active keys are reused; Clawboard does not rotate keys on each re-bootstrap.
- `clawboard.env` is written only when a new key is created.
...
Hierarchy is driven by agent metadata (`reportsTo`) stored in Clawboard and shown as a tree when
